...
class H5PValidator
{
    ...
    public function isValidPackage($skipContent = false, $upgradeOnly = false)
    {
        ...
        if ($this->h5pC->disableFileCheck !== true) {
            [$contentWhitelist, $contentRegExp] = $this->getWhitelistRegExp(false);
            [$libraryWhitelist, $libraryRegExp] = $this->getWhitelistRegExp(true);
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!empty($this->h5pC->maxFileSize) && $fileStat['size'] > $this->h5pC->maxFileSize) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'One of the files inside the package exceeds the maximum file size allowed. (%file %used > %max)',
                    ['%file' => $fileStat['name'], '%used' => ($fileStat['size'] / 1048576) . ' MB', '%max' => ($this->h5pC->maxFileSize / 1048576) . ' MB']
                ), 'file-size-too-large');
                $valid = false;
            }
            ...
            if ($fileName === 'h5p.json') {
                ...
                if ($skipContent === false && $this->h5pC->disableFileCheck !== true && !preg_match($contentRegExp, $fileName)) {
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        ...
                        ['%filename' => $fileStat['name'], '%files-allowed' => $contentWhitelist]
                    ), 'not-in-whitelist');
                    $valid = false;
                }
                ...
                if ($this->h5pC->disableFileCheck !== true && !preg_match($libraryRegExp, $fileName)) {
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        'File "%filename" not allowed. Only files with the following extensions are allowed: %files-allowed.',
                        ['%filename' => $fileStat['name'], '%files-allowed' => $libraryWhitelist]
                    ), 'not-in-whitelist');
                    $valid = false;
                }
        ...
        if (!empty($this->h5pC->maxTotalSize) && $totalSize > $this->h5pC->maxTotalSize) {
            ...
            $this->h5pF->setErrorMessage($this->h5pF->t(
                ...
                ['%used' => ($totalSize / 1048576) . ' MB', '%max' => ($this->h5pC->maxTotalSize / 1048576) . ' MB']
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!$fileStream) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'Unable to read file from the package: %fileName',
                    ['%fileName' => $fileName]
                ...
                if ($skipContent || !$mainDependency || !$this->h5pF->libraryHasUpgrade([
                    ...
                    foreach ($missingLibraries as $libString => $library) {
                        if (!empty($mainDependency) && $library['machineName'] === $mainDependency['machineName']) {
                            ...
                            $this->h5pF->setErrorMessage($this->h5pF->t(
                                'Missing required library @library',
                                ['@library' => $libString]
    ...
    private function getJson($path, $zip, $file, $assoc = false)
    {
        ...
        if ($json === null) {
            ...
            $this->h5pF->setErrorMessage($this->h5pF->t(
                'Unable to parse JSON from the package: %fileName',
                ['%fileName' => $file]
    ...
    public function getLibraryData($file, $filePath, $tmpDir)
    {
        ...
        if (file_exists($semanticsPath)) {
            ...
            if ($semantics === false) {
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'Invalid semantics.json file has been included in the library %name',
                    ['%name' => $file]
        ...
        if (is_dir($languagePath)) {
            ...
            foreach ($languageFiles as $languageFile) {
                ...
                if (preg_match('/^(-?[a-z]+){1,7}\.json$/i', $languageFile) === 0) {
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        'Invalid language file %file in library %library',
                        ['%file' => $languageFile, '%library' => $file]
                ...
                if ($languageJson === false) {
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        ...
                        ['%languageFile' => $languageFile, '%name' => $file]
                    ), 'invalid-language-file');
                    return false;
                }
    ...
    private function isExistingFiles($files, $tmpDir, $library)
    {
        foreach ($files as $file) {
            ...
            if (!file_exists($tmpDir . '/' . $library . '/' . $path)) {
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'The file "%file" is missing from library: "%name"',
                    ['%file' => $path, '%name' => $library]
        ...
        if (!empty($h5pData['coreApi'])) {
            if (($h5pData['coreApi']['majorVersion'] > H5PCore::$coreApi['majorVersion'])
                ...
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t(
                        'The system was unable to install the <em>%component</em> component from the package, it requires a newer version of the H5P plugin. This site is currently running version %current, whereas the required version is %required or higher. You should consider upgrading and then try again.',
                        [
    ...
    private function isValidRequirement($h5pData, $requirement, $library_name, $property_name)
    {
        ...
        if (is_string($requirement)) {
            if ($requirement == 'boolean') {
                if (!is_bool($h5pData)) {
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        'Invalid data provided for %property in %library. Boolean expected.',
                        ['%property' => $property_name, '%library' => $library_name]
                    ));
                    $valid = false;
                }
            ...
            if (is_array($h5pData)) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'Invalid data provided for %property in %library',
                    ['%property' => $property_name, '%library' => $library_name]
                ));
                $valid = false;
            }
            ...
            $this->h5pF->setErrorMessage($this->h5pF->t(
                "Can't read the property %property in %library",
                ['%property' => $property_name, '%library' => $library_name]
            ));
            $valid = false;
        }
    ...
    private function isValidRequiredH5pData($h5pData, $requirements, $library_name)
    {
        ...
        foreach ($requirements as $required => $requirement) {
            ...
            if (isset($h5pData[$required])) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'The required property %property is missing from %library',
                    ['%property' => $required, '%library' => $library_name]
    ...
    private function isValidH5pDataOptions($selected, $allowed, $library_name)
    {
        ...
        foreach ($selected as $value) {
            if (!in_array($value, $allowed)) {
                $this->h5pF->setErrorMessage($this->h5pF->t(
                    'Illegal option %option in %library',
                    ['%option' => $value, '%library' => $library_name]
...
class H5PStorage
{
    ...
    public $contentId; // Quick fix so WP can get ID of new content.
        ...
        if ($newOnes && $oldOnes) {
            if ($newOnes === 1) {
                if ($oldOnes === 1) {
                    // Singular Singular
                    $message = $this->h5pF->t(
                        'Added %new new H5P library and updated %old old one.',
                        ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                } else {
                    // Singular Plural
                    $message = $this->h5pF->t(
                        'Added %new new H5P library and updated %old old ones.',
                        ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                }
            } elseif ($oldOnes === 1) {
...
{
    public const CONTENT_TYPES = 'hub-api.h5p.org/v1/content-types/';
    public const SITES = 'hub-api.h5p.org/v1/sites';
...
class H5PCore
{
    ...
    public function __construct(H5PFrameworkInterface $H5PFramework, $path, $url, $language = 'en', $export = false)
    {
        ...
        $this->detectSiteType();
        $this->fullPluginPath = preg_replace('/\/[^\/]+\/?$/', '', __DIR__);
        ...
        while (!$available) {
            if ($available === false) {
                ...
                if (preg_match('/(.+-)([0-9]+)$/', $slug, $matches)) {
                    $slug = $matches[1] . ((int)($matches[2]) + 1);
                } else {
                    $slug .=  '-2';
                }
            }
        ...
        foreach (['dynamic', 'preloaded', 'editor'] as $type) {
            ...
            foreach ($library[$property] as $dependency) {
                ...
                if ($dependencyLibrary) {
                    ...
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        'Missing dependency @dep required by @lib.',
                        ['@dep' => H5PCore::libraryToString($dependency), '@lib' => H5PCore::libraryToString($library)]
    ...
    private function getDisplayOptionsAsArray($disable)
    {
        return [
            ...
            self::DISPLAY_OPTION_ABOUT => (bool)$this->h5pF->getOption(self::DISPLAY_OPTION_ABOUT, true),
        ];
    }
     ...
     * @param int|null $status_code Http response code
    ...
    public function checkSetupErrorMessage()
    {
        ...
        if ($max_upload_size < $byte_threshold) {
            $setup->errors[]
              = $this->h5pF->t(
                  'Your PHP max upload size is quite small. With your current setup, you may not upload files larger than %number MB. This might be a problem when trying to upload H5Ps, images and videos. Please consider to increase it to more than 5MB.',
                  ['%number' => number_format($max_upload_size / 1024 / 1024, 2, '.', ' ')]
              );
        }
        ...
        if ($max_post_size < $byte_threshold) {
            $setup->errors[]
              = $this->h5pF->t(
                  'Your PHP max post size is quite small. With your current setup, you may not upload files larger than %number MB. This might be a problem when trying to upload H5Ps, images and videos. Please consider to increase it to more than 5MB',
                  ['%number' => number_format($max_upload_size / 1024 / 1024, 2, '.', ' ')]
              );
        }
    ...
    public function hubAccountInfo()
    {
        ...
        $accountInfo = $this->h5pF->fetchExternalData(
            "{$url}/{$siteUuid}",
            null,
            true,
            null,
    ...
    public function getHubContentStatus($hubContentId, $syncStatus)
    {
        ...
        if ($contentStatus !== H5PContentStatus::STATUS_WAITING) {
            ...
            } elseif ((int)$syncStatus !== $contentStatus) {
...
class H5PContentValidator
{
    ...
    public function validateText(&$text, $semantics)
    {
        ...
        if (isset($semantics->tags)) {
            ...
            if (in_array('table', $tags)) {
                $tags = array_merge($tags, ['tr', 'td', 'th', 'colgroup', 'col', 'thead', 'tbody', 'tfoot', 'figure', 'figcaption']);
            }
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($filePath)) {
                ...
                if (!extension_loaded('mbstring')) {
                    ...
                    $this->h5pF->setErrorMessage($this->h5pF->t(
                        'File "%filename" not allowed. Only files with the following extensions are allowed: %files-allowed.',
                        ['%filename' => $file, '%files-allowed' => $whitelist]
                    ), 'not-in-whitelist');
                    $valid = false;
                }
            }
    ...
    private function _validateFilelike($file, $semantics, $typeValidKeys = [])
    {
        ...
        if (isset($file->width)) {
            $file->width = (int)($file->width);
        }

        if (isset($file->height)) {
            $file->height = (int)($file->height);
        }
        ...
        if (isset($file->quality)) {
            if (!is_object($file->quality) || !isset($file->quality->level) || !isset($file->quality->label)) {
                ...
                $file->quality->level = (int)($file->quality->level);
            ...
            foreach ($group as $key => &$value) {
                ...
                if ($found) {
                    if ($function) {
                        ...
                        // known validator.
                        $this->h5pF->setErrorMessage($this->h5pF->t(
                            'H5P internal error: unknown content type "@type" in semantics. Removing content!',
                            ['@type' => $field->type]
